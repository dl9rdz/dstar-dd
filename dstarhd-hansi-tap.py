...
#    global raw_header, symbole2;
    hdr=0
    lstr=''; al='';
    self.file = open("/home/jann/DStar/complex-record-512k-decode.pktn");
    pcap = open("out.pcap","wb");
    pcap.write(binascii.a2b_hex(pcap_global_header));

    TUNMODE = IFF_TAP
    tap = os.open("/dev/net/tun", os.O_RDWR);
    itap = ioctl(tap, TUNSETIFF, struct.pack("16sH", "dstar%d", TUNMODE))
    ifname = itap[:16].strip("\x00")
    print ("Allocated interface %s, waiting 20 seconds" % ifname)
    print ("Run 'ifconfig %s 33.33.33.33/30' and 'tcpdump -i %s' in a different window" % (ifname, ifname))
    time.sleep(20)
...
# write content in pcap format
	...
	data = pcaph + ''.join(format(d, '02X') for d in erg_pack)
	print ("data is ",data)
	dat = binascii.a2b_hex(data)
	pcap.write(dat) 

        data = "00000806"+''.join(format(d, '02X') for d in erg_pack)
        os.write(tap,binascii.a2b_hex(data))
	time.sleep(1)
